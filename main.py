...
if STRATEGIE_ACTIVE is None:
    ...
    try:
        ...
        strategy_result = module.run_strategy(df)

        if isinstance(strategy_result, dict):
            strategy_trades = strategy_result.get("trades", [])
            strategy_zones = strategy_result.get("zones", [])
        else:
            strategy_trades = strategy_result
            strategy_zones = []

        print(f"📊 Trades générés : {len(strategy_trades)}")
        print(f"🧱 Zones générées : {len(strategy_zones)}")
...
if strategy_trades:
    ...
    longs = sum(
        1 for t in strategy_trades
        if isinstance(t, dict) and str(t.get("type", "")).strip().lower() == "long"
    )
    shorts = sum(
        1 for t in strategy_trades
        if isinstance(t, dict) and str(t.get("type", "")).strip().lower() == "short"
    )

    tp = sum(
        1 for t in strategy_trades
        if isinstance(t, dict) and str(t.get("exit_reason", "")).strip().upper() == "TP"
    )
    sl = sum(
        1 for t in strategy_trades
        if isinstance(t, dict) and str(t.get("exit_reason", "")).strip().upper() == "SL"
    )
    ...
    print(f" ✅ TP : {tp} ({round(tp / total * 100, 1) if total > 0 else 0.0}%)")
    print(f" ❌ SL : {sl} ({round(sl / total * 100, 1) if total > 0 else 0.0}%)")
...
strategy_zones_javascript = json.dumps(strategy_zones, ensure_ascii=False)
...
html_content = html_content.replace('{{strategy_zones_javascript}}', strategy_zones_javascript)
